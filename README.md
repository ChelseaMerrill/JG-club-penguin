...
`e2e/presence-two-browsers.spec.ts` and `e2e/chat-two-browsers.spec.ts` sign in the shared Supabase project's two test users, A and B (`E2E_USER_A_EMAIL`/`_PASSWORD`, `E2E_USER_B_EMAIL`/`_PASSWORD` in `.env.test.local`, or `AUTH_STATE_A`/`AUTH_STATE_B` storage-state files). They run against the **real shared Supabase project** (#81), so other Players are expected to be in Town Center at the same time — these specs must never check a roster's total count, only specific test-user Player ids. Only one session at a time may run this pair of test users through either spec: both call a pre-flight guard (`e2e/support/presence-guard.ts`) before opening any browser context, which fails fast if a test user is still in Town Center from another session. Playwright already runs both specs in the single-worker `realtime-shared-users` project (`playwright.config.ts`), so they never race each other within one `npm run test:e2e` run.
